...
API_CONTRIBUTION = "https://www.showroom-live.com/api/event/contribution_ranking" # 既存ロジック維持のため残す
...
# 管理者モードのフィルタリング基準 (2023年9月1日 00:00:00 JST)
ADMIN_FILTER_START_TS = FILTER_START_TS
# --------------------
...
# ⑤終了日時が当日の場合のハイライトがされてないです。 -> 既存定義を流用
...
def load_event_db(url):
    try:
        ...
        # ★★★ 既存コードの修正: dtype=object で読み込み、欠損値を' 'に置換 ★★★
...
# ★★★ ①貢献ランクへのリンク表示は不要です（項目ごと不要）に伴い、
# fetch_contribution_rank は既存ロジックで利用されないため削除しても良いが、今回は既存コード維持のため残す
def fetch_contribution_rank(*args, **kwargs):
    # 既存のロジックから変更なし (ライバーモードで利用しないため)
    return []

# ---------- 貢献ランクURL生成ロジック (管理者モードでのみ利用) ----------
...
if 'admin_start_date_filter' not in st.session_state:
    st.session_state.admin_start_date_filter = "全期間" # セッションに保存
if 'admin_end_date_filter' not in st.session_state:
    st.session_state.admin_end_date_filter = "全期間" # セッションに保存
...
if is_admin:
    # --- 管理者モードのデータ処理 ---
    # ★★★ ②管理者モードの表記修正: フィルタリング状態に関わらず「管理者モード」のみ表記 ★★★
    st.info(f"**管理者モード**")
    ...
    # ⑤終了日時が当日の場合のハイライトがされてないです。 -> 終了日時が今日0時〜明日0時の間
    tomorrow_ts = today_ts + 86400
    df["is_end_today"] = df["__end_ts"].apply(lambda x: pd.notna(x) and today_ts <= x < tomorrow_ts)


    # 3. UIコンポーネント (フィルタ、最新化ボタン)
    # ★★★ ⑥表示バランス調整 & ③プルダウン配置変更 ★★★
    col1, col2, col3, col4 = st.columns([1.5, 1.3, 1.3, 1.5]) # 比率を調整

    # 最新化ボタン
    with col1:
        st.button("🔄 開催中イベントの最新化", on_click=refresh_data, key="admin_refresh_button")

    # 開始日時フィルタリング (③プルダウン配置変更: 左側)
    ...
    if not st.session_state.admin_full_data:
        # ★★★ ④全量表示の際、各ライバー用同様に、2023年9月1日以降に開始したイベントを表示してください。★★★
        # 開始日時がADMIN_FILTER_START_TS以降のイベントに絞り込み
        df_filtered = df_filtered[
            (df_filtered["__start_ts"].apply(lambda x: pd.notna(x) and x >= ADMIN_FILTER_START_TS))
    ...
    if st.session_state.get('refresh_trigger', False):
        ...
        with st.spinner("開催中イベントの順位/ポイントを最新化中..."):
            for idx, row in ongoing.iterrows():
                ...
                if stats:
                    # df_filtered ではなく、元の df_all (セッション) の対応する行も更新することで、
                    # 次回以降のフィルタリングでも最新情報が使われるようにする（ただし、今回はdf_allの更新を省略し、df_filteredのみ更新）
                    # 既存コードが df_all の更新をしていないため、ここでは df_filtered の更新のみとする
    ...
    # 7. 表示整形
    # ★★★ ①貢献ランクURLは不要だが、管理者モードは URL 列（貢献ランクボタン）を残すため、disp_colsにURLを残す ★★★
    ...
    # 3. 2023年9月1日以降のイベントにフィルタリング (既存ロジック維持)
    ...
    # 6. ポイントランキングを計算し、ハイライトCSSを決定するロジック (既存ロジック維持)
    ...
    # ★★★ ①貢献ランク関連の要素削除: 'URL'と'event_id'と、貢献ランクボタン表示に関連する列を削除 ★★★
...
def make_html_table_user(df, room_id):
    """ライバー用HTMLテーブルを生成（貢献ランクボタン風リンクなし、ポイントハイライトあり、開催中黄色ハイライト）"""
    html = """
    <style>
    ...
    /* ★★★ ①貢献ランク項目削除に伴うカラム幅調整 ★★★ */
    table col:nth-child(1) { width: 50%; } /* イベント名 */
    table col:nth-child(2) { width: 13%; } /* 開始日時 */
    table col:nth-child(3) { width: 13%; } /* 終了日時 */ 
    ...
    for _, r in df.iterrows():
        ...
        html += f"<td>{r['順位']}</td>{point_td}<td>{r['レベル']}</td>"
        html += "</tr>"
...
# ----------------------------------------------------------------------
# HTMLテーブル生成関数 (管理者モード用 - 既存ロジック維持)
# ----------------------------------------------------------------------
def make_html_table_admin(df):
    """管理者用HTMLテーブルを生成（ライバー名列あり、ポイントハイライトなし、終了当日ハイライトあり）"""
    # ★★★ ①貢献ランクへのリンク表示は不要です（項目ごと不要）。なぜか見出しが「URL」となっていますが、見出しごと不要です。
    # -> 管理者モードでは貢献ランクボタンを残すため、<th>URL</th>を残す ★★★
    html = f"""
    <style>
    ...
    thead th {{ position: sticky; top: 0; background: #0b66c2; color: #fff; padding: 5px; text-align: center; border: 1px solid #0b66c2; z-index: 10; }}
    tbody td {{ padding: 5px; border-bottom: 1px solid #f2f2f2; text-align: center; vertical-align: middle; word-wrap: break-word; }}
    /* ★★★ 管理者用: カラム幅の指定（ライバー名追加、貢献ランクボタン列） ★★★ */
    ...
    for _, r in df.iterrows():
        ...
        url = url_value if pd.notna(url_value) and url_value else ""
        room_id_for_link = room_id_value if pd.notna(room_id_value) and room_id_value else ""
        ...
        # ライバー名リンク (別タブ)
        liver_link_url = f"https://www.showroom-live.com/room/profile?room_id={room_id_for_link}"
        liver_link = f'<a class="liver-link" href="{liver_link_url}" target="_blank">{liver_name}</a>' if room_id_for_link else liver_name

        # 貢献ランクURLを生成し、ボタン風リンクにする
        contrib_url = generate_contribution_url(url, room_id_for_link)
        if contrib_url:
...
if is_admin:
    ...
    # HTMLテーブルで表示 (make_html_table_user に変更)
